...
const RED='#d60000',BG='#0a0a0a',CARD='#111',BORDER='#1e1e1e',MUTED='#555';
    ...
    <div style={{background:CARD,border:'1px solid '+BORDER,borderRadius:6,padding:'18px 20px',marginBottom:10,display:'flex',gap:16,alignItems:'flex-start'}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:'4px 16px',fontSize:12,color:'#666'}}>
          ...
          {(lead.landing_page||lead.page_url)&&<span style={{color:'#444'}}>🛬 {lead.landing_page||lead.page_url}</span>}
        </div>
      </div>
...
export default function LeadsFeed({session}){
  ...
  const btn=(val,label,count)=><button onClick={()=>setFilter(val)} style={{background:filter===val?RED:'transparent',border:'1px solid '+(filter===val?RED:'#2a2a2a'),borderRadius:4,padding:'6px 14px',color:filter===val?'#fff':'#888',fontSize:12,fontFamily:'Oswald,sans-serif',fontWeight:600,letterSpacing:'0.05em',cursor:'pointer'}}>{label}{count>0&&<span style={{opacity:0.7}}> ({count})</span>}</button>;

  return(
    <div style={{background:BG,minHeight:'100vh',fontFamily:'Barlow,sans-serif'}}>
      <div style={{background:'#0d0d0d',borderBottom:'1px solid '+BORDER,padding:'14px 28px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div>
          <span style={{fontFamily:'Oswald,sans-serif',fontSize:18,fontWeight:600,color:'#fff',letterSpacing:'0.05em'}}>FORTITUDE</span>
          <span style={{fontSize:12,color:MUTED,marginLeft:10}}>Leads Intelligence</span>
        </div>
        <button onClick={()=>supabase.auth.signOut()} style={{background:'transparent',border:'1px solid #2a2a2a',borderRadius:4,padding:'5px 12px',color:MUTED,fontSize:12,cursor:'pointer'}}>Sign out</button>
      </div>
      <div style={{maxWidth:860,margin:'0 auto',padding:'28px 20px'}}>
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:20}}>
          {btn('all','ALL',counts.all)}{btn('rb2b','RB2B',counts.rb2b)}{btn('snitcher','SNITCHER',counts.snitcher)}
          <div style={{flex:1}}/>
          <button onClick={fetchLeads} style={{background:'transparent',border:'1px solid #2a2a2a',borderRadius:4,padding:'6px 12px',color:MUTED,fontSize:12,cursor:'pointer'}}>↻ Refresh</button>
        </div>
        {error&&<div style={{background:'#1a0000',border:'1px solid #3a0000',borderRadius:4,padding:'12px 16px',color:'#ff6b6b',fontSize:13,marginBottom:16}}>{error}</div>}
        {loading?<div style={{textAlign:'center',color:MUTED,padding:60,fontSize:13}}>Loading leads...</div>
        :leads.length===0?<div style={{textAlign:'center',color:MUTED,padding:60,fontSize:13}}>No leads yet. They will appear here as visitors are identified.</div>
